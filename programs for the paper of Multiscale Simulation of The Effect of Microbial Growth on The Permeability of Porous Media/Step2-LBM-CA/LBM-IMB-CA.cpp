...
const int gujiaN=30;//Number of grains
...
double u1_phy = 1.278e-4*0.94;// The actual velocity "m/s" is obtained from the REV calculation
double inlet_concf_phy = 4e-3*0.4972;//Left side inlet nutrient concentration "kg/m^3" 
...
AA:
  printf("Input the number of days (integer) for microbial growth :\n");
  scanf("%d",&day);
...
{     
	  ...
	  for(i=1;i<=n;i++) 
	   ...
	   { 
	    conshow[i][j]=0; 
        if(M[i][j]>0) { conshow[i][j]=1; Sumes=Sumes+1;} 
		if(es00[i][j]>0) conshow[i][j]=solidc;
...
void permeabilityK(int njs)//Permeability
{
   ...
   double spout=0.0;
    ...
    pin1=spin/m; pout1=spout/m;
	 
	double K1=(niu*rho0*(n-1)*u1)/(pin1-pout1);
